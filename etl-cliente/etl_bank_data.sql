...
SELECT
  ...
  -- KPI conti
  IFNULL(co.num_conti, 0)                          AS num_conti,
  IFNULL(co.num_conti_attivi, 0)                   AS num_conti_attivi,
  IFNULL(co.saldo_totale, 0)                       AS saldo_totale,
  IFNULL(co.fido_totale, 0)                        AS fido_totale,
  IFNULL(co.anni_anzianita, 0)                     AS anni_anzianita,

  -- KPI movimenti 12m
  IFNULL(mv.num_movimenti_12m, 0)                  AS num_movimenti_12m,
  IFNULL(mv.tot_entrate_12m, 0)                    AS tot_entrate_12m,
  IFNULL(mv.tot_uscite_12m, 0)                     AS tot_uscite_12m,
  IFNULL(mv.saldo_netto_12m, 0)                    AS saldo_netto_12m,
  IFNULL(mv.num_stipendi, 0)                       AS num_stipendi,
  IFNULL(mv.tot_stipendi, 0)                       AS tot_stipendi,
  IFNULL(mv.num_pag_carta, 0)                      AS num_pag_carta,
  IFNULL(mv.num_investimenti, 0)                   AS num_investimenti,
  IFNULL(mv.tot_investimenti, 0)                   AS tot_investimenti,
